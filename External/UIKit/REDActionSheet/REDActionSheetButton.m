...
//				self.backgroundColor = [UIColor colorWithRed:0.204 green:0.204 blue:0.204 alpha:1];
                self.backgroundColor = [UIColor colorWithRed:0.811 green:0.09 blue:0.145 alpha:1];
				[self setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
				break;
...
- (void)drawRect:(CGRect)rect
{
	...
	{
		path = [UIBezierPath bezierPathWithRoundedRect:rect byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(0, 0)];
	}
	else if (self.position == REDActionSheetButtonPositionBottom)
	{
		path = [UIBezierPath bezierPathWithRoundedRect:rect byRoundingCorners:UIRectCornerBottomLeft | UIRectCornerBottomRight cornerRadii:CGSizeMake(0, 0)];
	}
	else if (self.type == REDActionSheetButtonTypeCancel)
	{
		path = [UIBezierPath bezierPathWithRoundedRect:rect cornerRadius:0];
	}
