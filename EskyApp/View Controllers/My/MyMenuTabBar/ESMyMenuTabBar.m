...
- (void)addBg
{
    ...
    for (id view in [_customView subviews]) {
        if ([view isKindOfClass:[ESMyMenuBarItem class] ]) {
            _itemCount ++ ;
...
{
    ESMyMenuBarItem *item = [self menuItem: index];
    item.block =block;
}
...
- (ESMyMenuBarItem *)menuItem:(NSInteger)index
{
    int i = index+kMenuItemTagBegin;
    id view = [self.customView viewWithTag:i];
    if ([view isKindOfClass:[ESMyMenuBarItem class] ]) {
        return (ESMyMenuBarItem *)view;
    }else{
...
{
    for (NSInteger i = 0 ; i < _itemCount; i ++) {
        ESMyMenuBarItem *item = [self menuItem: i];
        if (item.tag != tag) {
            item.isActive = NO ;
        }
    }
